...
addresses = pd.read_csv('./data/bLuna_addresses.csv')['0'].values
addr_balances = pd.read_csv('./data/balances/bLuna.csv').address.values
addresses = set(addresses).difference(set(addr_balances))
while(len(addresses) > 0):
    print(f'Left to query: {len(addresses)} addresses')
    results = perform_web_requests('terra1kc87mu460fwkqte29rquh4hc20m54fxwtsx7gp',list(addresses)[:100], 1)
    balances = pd.DataFrame(results, columns=['address','bLuna_balance'])
    balances.to_csv('./data/balances/bLuna.csv',mode='a',header=False)
    addr_balances = pd.read_csv('./data/balances/bLuna.csv').address.values
    addresses = set(addresses).difference(set(addr_balances))
